...
public class BusinessAction extends ActionSupport {
	...
	public String savebusiness() throws AppException {
		...
		try {
			...
			if (phone_begin != null && !phone_begin.equals("")
					&& phone_end != null && !phone_end.equals("")) {
			...
			if (invoicetel_begin != null && !invoicetel_begin.equals("")
					&& invoicetel_end != null && !invoicetel_end.equals("")) {
			...
			while (configit.hasNext()) {
				...
				if (config.getName() != null
						&& config.getName().equals("INSPECTION1")) {
					bus.setInspection1(config.getValue());
				} else if (config.getName() != null
						&& config.getName().equals("INSPECTION2")) {
					bus.setInspection2(config.getValue());
				} else if (config.getName() != null
						&& config.getName().equals("INSPECTIONDATE")) {
	...
	public String apply() throws AppException {
		...
		try {
			...
			List<BusinessDesCountry> busList = businessService
					.getBusinessDesCountry(Long.valueOf(id));
			String countrysandports_input = ""; // 目的国及港口
			if (busList != null && busList.size() > 0) {
				for (int i = 0; i < busList.size(); i++) {
					BusinessDesCountry businessDesCountry = busList.get(i);
					if (businessDesCountry.getSelectindex() != null
							&& !businessDesCountry.getSelectindex()
									.equals("-1")) {
						String tempport = "";
						if (businessDesCountry.getPort() != null
								&& !businessDesCountry.getPort().equals("")) {
							// tempport = businessDesCountry.getPort() +" / ";
						}
						countrysandports_input = countrysandports_input
								+ tempport + businessDesCountry.getCountry()
								+ "; ";
					}
				}
				countrysandports_input = countrysandports_input.substring(0,
						countrysandports_input.length() - 2);
			}
			...
			vesselnameandtype_input = bus.getVesselname() + " / "
					+ bus.getVesseltype();
			...
			reg_imo_tonnage_input = regis_input + " / " + bus.getImo() + " / "
					+ tonn + " MT";

			String inoutdate_input = ""; // 计划入港 出港日期
			inoutdate_input = dateFormat.format(bus.getPlandatein()) + " / "
					+ dateFormat.format(bus.getPlandateout());
			...
			if (portlist != null && portlist.size() > 0) {
				...
				String bre = bus.getBerth() == null ? "" : bus.getBerth()
						+ ", ";
				breth_inpit = bre + port_sname + ", " + cityname.toUpperCase()
						+ ", " + BusinessState.APPLY_CHINA;
			}
			...
			request.setAttribute("businessname", bus.getBusinessname()
					.toUpperCase());
			...
			request.setAttribute("countrysandports_input",
					countrysandports_input.toUpperCase());
			request.setAttribute("vesselnameandtype_input",
					vesselnameandtype_input.toUpperCase());
			request.setAttribute("reg_imo_tonnage_input",
					reg_imo_tonnage_input.toUpperCase());
	...
	public String payment() throws AppException {
		...
		try {
			...
			List tonlist = businessService.getShipPaymentList(bus
					.getVesseltype());
	...
	public String businessApply() {
		...
		Locale local_language = (Locale) session
		...
		try {
			...
			businessnamelist = clientService.getClientListById(client
					.getClientId());
	...
	public String ajaxSelect() {
		...
		try {
			if (selecttype.equals("0")) { // 按省的ID查询下属市
				...
				if (cityid != null) {
					portList = registerService.getPortlistByProandCity(
							promaryid, cityid);
				}
	...
	public String getBusinessByPage() throws AppException {
		...
		try {
			Account account = (Account) sessionmap
					.get(Constants.ACCOUNT_SESSION);
			String accounttype = account.getAccountType().toString();
			businessstate = Long.valueOf(BusinessState.BUSINESS_TEMP);
			map.put("state", businessstate);
			List list = null;

			if (accounttype != null && accounttype.equals("1")) { // 用户
																	// 查看本机构及本机构下级机构的内容
				String is_child = (String) sessionmap
						.get(Constants.ACCOUNT_PERSON_IS_CHILD); // 得到用户机构等级
				String org4 = (String) sessionmap
						.get(Constants.ACCOUNT_ORG_ID4); // 当前用户的4位机构ID
				...
				list = businessService.getBusinessList(begin, numOfEachPage,
						map);
			} else if (accounttype != null && accounttype.equals("2")) { // 客户
																			// 查看自己提交的内容
				Client client = (Client) sessionmap
						.get(Constants.ACCOUNT_CLIENT);
				String clientname = client.getCoEnName();
				map.put("businessname", clientname);
				Integer totleCount = businessService
						.getBusinessListClientCOUNT(map);
				p.getPagination(request, totleCount, currPage1, null, null,
						null);
				int numOfEachPage = Constants.NUMOFEACHPAGE;
				int begin = p.getRownum_begin(numOfEachPage, currPage1);
				list = businessService.getBusinessListClient(begin,
						numOfEachPage, map);
			}
	...
	public String searchBusiness() throws Exception {
		...
		if (portcity_form != null && !portcity_form.equals("")
				&& !portcity_form.equals("-1")) {
	...
	public String commit() throws Exception {
		...
		try {
			...
			bus.setAppdate(date);
			bus.setBerth(berth);
			bus.setBusinessstate(Long
					.valueOf(BusinessState.BUSINESS_ACCEPTWAIT));
			bus.setCertification(certification);
			bus.setCheckdate(checkdate);
			bus.setCompany(company);
			// bus.setDestinationcountry(destinationcountry);
			...
			if (temp == null || temp.getBusinessstate() == 0) { // 为防止重复提交，只在新保存、未提交的单子的情况下才会提交
				...
				if (bus.getInspection1() == null
						|| "".equals(bus.getInspection1())) {
			...
			List procityList = registerService.getPromaryAndCityByPortID(bus
					.getPortid());
			if (procityList != null && procityList.size() > 0) {
				...
				List procityportnameList = registerService.getPCPNameByPortID(
						proid, cityid, bus.getPortid());
				if (procityportnameList != null
						&& procityportnameList.size() > 0) {
	...
	public String commitaftersave() throws AppException {
		...
		try {
			...
			bus.setBusinessstate(Long
					.valueOf(BusinessState.BUSINESS_ACCEPTWAIT));
			List<BusinessDesCountry> businessDesCountryList = businessService
					.getBusinessDesCountry(bus.getId());
			String date_str = dateFormat.format(date);
			setBusinessCountrysandPorts(businessDesCountryList, request);
	...
	public String updataApply() throws AppException {
		...
		try {
			...
			List procityList = registerService
					.getPromaryAndCityByPortID(business.getPortid());
			if (procityList != null && procityList.size() > 0) {
				HashMap procitymap = (HashMap) procityList.get(0);
				BigDecimal cityid_b = (BigDecimal) procitymap.get("CITYID"); // 港口所在最下级城市
				BigDecimal proid_b = (BigDecimal) procitymap.get("PROID"); // 港口所在省份
	...
	public String pass() throws AppException {
		...
		try {
			...
			int numOfEachPage = Constants.NUMOFEACHPAGE;
			int begin = p.getRownum_begin(numOfEachPage, currPage1);
			List<Business> list = businessService.getAllBusiness(begin,
					numOfEachPage);
	...
	public String getAllotPage() {
		...
		try {
			Account account = (Account) sessionmap
					.get(Constants.ACCOUNT_SESSION);
			String personid = (String) sessionmap
					.get(Constants.ACCOUNT_PERSON_ID);
			...
			if (isadmin != null && isadmin.equals("1")) { // 系统管理员用户
				Integer totleCount = assignmentService
						.getBusinessByPersonIDCountSYS(map);
				p.getPagination(request, totleCount, currPage1, null, null,
						null);
				int numOfEachPage = Constants.NUMOFEACHPAGE;
				int begin = p.getRownum_begin(numOfEachPage, currPage1);
				list = assignmentService.getBusinessByPersonIDSYS(begin,
						numOfEachPage, map);

			} else if (account.getAccountType() == 1) { // 如果为用户 person
														// (除去系统管理员)

				Integer totleCount = assignmentService
						.getBusinessByPersonIDCount(personid, map);
				p.getPagination(request, totleCount, currPage1, null, null,
						null);
				int numOfEachPage = Constants.NUMOFEACHPAGE;
				int begin = p.getRownum_begin(numOfEachPage, currPage1);
				list = assignmentService.getBusinessByPersonID(begin,
						numOfEachPage, personid, map);
			} else { // 客户 client
				// do nothing
			}
			...
			request.setAttribute("appdate_begin", appdate_begin);
			request.setAttribute("appdate_end", appdate_end);
			request.setAttribute("vesselname_form",
					StringUtils.codeHtml(vesselname_form));
			request.setAttribute("portcity_form",
					StringUtils.codeHtml(portcity_form));
			request.setAttribute("appno_form", StringUtils.codeHtml(appno_form));
			request.setAttribute("allotstate_hide",
					StringUtils.codeHtml(allotstate_form));
	...
	public String getAllotPageDetail() {
		...
		try {
			if (account_type != null && account_type == 1) { // 如果为 用户(Person)
				...
				String allotright = (String) sessionmap
						.get(Constants.ACCOUNT_ALLOT_LEVEL);

				business = businessService.getBusinessById(Long
						.valueOf(business_id));
				List assignlist = assignmentService
						.getAssignmentlistByBusinessId(business.getId());
				String assignstate = "";
				...
				List<BusinessDesCountry> businessDesCountryList = businessService
						.getBusinessDesCountry(business.getId());
				setBusinessCountrysandPorts(businessDesCountryList, request);

				List procityList = registerService.getPortProCity(business
						.getPortid());
				if (procityList != null && procityList.size() > 0) {
		...
		try {
			...
			if (account_type != null && account_type == 1) { // 如果为 用户(Person)
				...
				business = businessService.getBusinessById(Long
						.valueOf(business_id));
				request.setAttribute("business_id", business_id);
				list = assignmentService
						.getAssignmentWorkflow(business.getId());
	...
	public String toAllot() {
		...
		String curr_is_child = (String) sessionmap
				.get(Constants.ACCOUNT_PERSON_IS_CHILD);

		_logger.info("toAllot begin");

		String allottype = request.getParameter("allottype"); // 0:分配协调员 ,
																// 1:分配授权签字人 检查员
		Account account = (Account) session
				.getAttribute(Constants.ACCOUNT_SESSION);
		Date date = new Date();
		...
		String currindex = "";
		try {
			List numberlist = assignmentService
					.getAssignmentlistByBusinessId(Long.valueOf(businessid));
			...
			if (allottype.equals("0")) { // 分配协调员
				...
				if (list != null && list.size() > 0) {
					...
					if (is_child.equals("1")) { // 二级协调员
						assignerstate = Long
								.valueOf(BusinessState.Allot_SECOND_COOR);
					} else if (is_child.equals("2")) { // 三级协调员
						assignerstate = Long
								.valueOf(BusinessState.Allot_THIRD_COOR);
					}
					...
					assignment.setAssignumber(Long.valueOf(currindex)); // 设置当前操作数
																		// +1
					...
					Payment payment = chargesService
							.getPaymentByBusinessID(Long.valueOf(businessid));
					if (is_child.equals("1")) {
						payment.setFirstcomid(orgfrom);
						payment.setSecondcomid(orgto);
						chargesService.updatePayment(payment);
					} else if (is_child.equals("2")
							&& curr_is_child.equals("0")) { // 由系统管理员分配
						payment.setFirstcomid(orgfrom);
						OrganizationLevel organizationLevel = organizationService
								.getOrgLevelbyId(orgto);
						payment.setSecondcomid(organizationLevel.getParentId());
						chargesService.updatePayment(payment);
					} else if (is_child.equals("2")
							&& curr_is_child.equals("1")) { // 由一级协调员分配
						OrganizationLevel organizationLevelfrom = organizationService
								.getOrgLevelbyId(orgfrom);
						payment.setFirstcomid(organizationLevelfrom
								.getParentId());
						OrganizationLevel organizationLevel = organizationService
								.getOrgLevelbyId(orgto);
						payment.setSecondcomid(organizationLevel.getParentId());
				...
				if (list != null && list.size() > 0) {
					...
					if (is_child_auth.equals("1")) { // 二级检查人、授权签字人
						assignerstate = Long
								.valueOf(BusinessState.Allot_SECOND_INS);
					} else if (is_child_auth.equals("2")) { // 三级检查人、授权签字人
						assignerstate = Long
								.valueOf(BusinessState.Allot_THIRD_INS);
					}
				...
				for (int i = 0; i < ins_input_id.length; i++) {
					...
					List inslist = businessService
							.getOrgByPersonID(ins_input_id[i]);
					long insstate = 1;
					String insorgto = "";
				...
				Business business = businessService.getBusinessById(Long
						.valueOf(businessid));
				business.setBusinessstate(Long
						.valueOf(BusinessState.Business_ALLOTED));
	...
	public String reAllot() {
		...
		try {
			...
			assignment.setAssigntime(date);
			assignment
					.setAssignerstate(Long.valueOf(BusinessState.Allot_FIRST));
			assignment.setBusinessid(Long.valueOf(businessid));
			assignment.setIscomplete(BusinessState.LONG_ZERO); // 设置当前业务单为未完成分配
			assignment.setIsapplay(BusinessState.LONG_ONE); // 设置当前状态为已激活生效
	 ...
	 * 分配权限分两部分: <br/>
	 * 一: 分配协调员 <br/>
	 * 1.属于CCIC 一级机构 的用户可以分配 下级 及 下下级的 <b>协调员</b> <br/>
	 * 2.属于(河北...) 二级机构 的用户只可以分配 下级的<b>协调员</b> <br/>
	 * 3.属于(沧州...) 三级机构 的用户不能分配 <b>协调员</b> <br/>
	 * <br/>
	 * <br/>
	 * 二: 分配检查员(授权签字人) <br/>
	 * 1.属于CCIC 一级机构 的用户 <b>不能</b> 分配 <b>检查员</b> <br/>
	 * 2.属于(河北...) 二级机构 的用户可以分配 同级 及 下级的 <b>检查员</b> <br/>
	 * 3.属于(沧州...) 三级机构 的用户只能分配同级的 <b>检查员</b> <br/>
	 * 
	 * @return
	 * @param
	 * 
	...
	public String allotlist() {
		...
		String radio_check = request.getParameter("radio_check"); // 分配对象为 0:协调员
																	// /
																	// 1:检查员授权签字人
		String SESSION_ORGLEVEL = (String) sessionmap
				.get(Constants.ACCOUNT_PERSON_IS_CHILD); // 所属机构等级
		...
		try {
			...
			List list = businessService.getOrganizationLevelByOrgID(orgid,
					condition);
			...
			if (first_is_child.equals("1")) { // 如果第一个分支是1级子公司 河北公司
												// 则需要把他的父位置信息改为0 ,这样树形控件才能显示
	...
	/**
	 * 显示机构树及权限分配人员界面 根据机构ID得到指定的机构的用户类型
	 * 
	 * @return
	 * @param
	 * 
	...
	public String allotlistbyorg() {
		...
		try {
			...
			} else if (first_is_child.equals("2")) { // 如果第一个分支是2级子公司 沧州分公司
														// 则需要把父位置信息改为0
														// ,这样树形控件才能显示
				for (int i = 0; i < list.size(); i++) {
					HashMap map = (HashMap) list.get(i);
	...
	public String tableContent() {
		HttpServletRequest request = ServletActionContext.getRequest();
		String orgid = request.getParameter("orgid");
		String allottype = request.getParameter("allottype"); // 分配对象类型 0:协调员
																// /1:授权签字人
																// /2:检查员
	...
	public String getAllotPageView() {
		...
		try {
			...
			vesselnameandtype_view = business.getVesselname() + " / "
					+ business.getVesseltype();
			...
			reg_imo_tonnage_view = regis_input + " / " + business.getImo()
					+ " / " + tonn + " MT";

			String plandateout_view = dateFormat.format(business
					.getPlandateout()); // 计划离港日期
			String checkdate_view = dateFormat.format(business.getCheckdate()); // 检查日期

			List<BusinessDesCountry> busdesCountrylist = businessService
					.getBusinessDesCountry(business.getId());
			String countrysandports_input = "";
			for (int i = 0; i < busdesCountrylist.size(); i++) {
				BusinessDesCountry businessDesCountry = busdesCountrylist
						.get(i);
				if (businessDesCountry.getSelectindex() != null
						&& !businessDesCountry.getSelectindex().equals("-1")) {
					String tempport = "";
					if (businessDesCountry.getPort() != null
							&& !businessDesCountry.getPort().equals("")) {
						// tempport = businessDesCountry.getPort()+" / ";
					}
					countrysandports_input = countrysandports_input + tempport
							+ businessDesCountry.getCountry() + " ; ";
				}
			}
			...
			String breth_inpit = ""; // 船舶及港口城市 breth, tianjin, 天津, CHINA
			List portlist = registerService
					.getPortProCity(business.getPortid());
			if (portlist != null && portlist.size() > 0) {
				HashMap portmap = (HashMap) portlist.get(0);
				String port_sname = (String) portmap.get("PORT_SNAME");
				String cityname = (String) portmap.get("CITYNAME");
				// String bre =
				// business.getBerth()==null?"":business.getBerth()+", ";
				String bre = "";
				if (!cityname.toUpperCase().equals(port_sname.toUpperCase())) {
					breth_inpit = bre + port_sname + ", "
			...
			String auth_input = "";
			List authlist = assignmentService
					.getPersonOnAssignment(business.getId().toString(),
							String.valueOf(BusinessState.LONG_ONE));
			...
			request.setAttribute("businessname_view",
					businessname_view.toUpperCase());
			request.setAttribute("company_view", company_view.toUpperCase());
			request.setAttribute("vesselnameandtype_view",
					vesselnameandtype_view.toUpperCase());
			request.setAttribute("reg_imo_tonnage_view",
					reg_imo_tonnage_view.toUpperCase());
			request.setAttribute("plandateout_view", plandateout_view);
			request.setAttribute("checkdate_view", checkdate_view);
			request.setAttribute("countrysandports_input",
					countrysandports_input.toUpperCase());
			request.setAttribute("breth_inpit", breth_inpit.toUpperCase());
			request.setAttribute("auth_input",
					PinYin.getPingYin(auth_input, false, false).toUpperCase());
			request.setAttribute("certid_view", business.getCertid());

			// ================== add by guodingjun in 2011-03-31 ==============
			// ================== add star ==============
			// 如果没从数据库取得检查方法，就从 language_zh_CN.properties文件当中获取到检查方法
			if (business.getInspection1() == null
					|| "".equals(business.getInspection1())) {
				ResourceBundle rb = PropertyResourceBundle.getBundle(
						"language", Locale.CHINA);
				request.setAttribute("inspection_input",
						rb.getString("INSPECTION1"));
			} else {
				request.setAttribute("inspection_input",
						business.getInspection1());
			}
			// ================== add end ===================
	...
	public String acceptPassdetail() {
		...
		try {
			...
			list.add(business);
			List<BusinessDesCountry> busdesCountrylist = businessService
					.getBusinessDesCountry(Long.parseLong(id));
			...
			if (procityList != null && procityList.size() > 0) {
				...
				request.setAttribute("select_cityid", cityid);
				List procityportnameList = registerService.getPCPNameByPortID(
						proid, cityid, business.getPortid());
				if (procityportnameList != null
						&& procityportnameList.size() > 0) {
					HashMap namemap = (HashMap) procityportnameList.get(0);
					String proname = (String) namemap.get("PRONAME");
	...
	public String getBusinessById() throws Exception {
		...
		List procityList = registerService.getPromaryAndCityByPortID(business
				.getPortid());
		if (procityList != null && procityList.size() > 0) {
			HashMap procitymap = (HashMap) procityList.get(0);
			BigDecimal cityid_b = (BigDecimal) procitymap.get("CITYID"); // 港口所在最下级城市
	...
	public void showAccountname(String accountid) throws Exception {
		HttpServletRequest request = ServletActionContext.getRequest();
		Map sessionmap = ActionContext.getContext().getSession();
		Account curr_account = (Account) sessionmap
				.get(Constants.ACCOUNT_SESSION);
		if (curr_account.getAccountType().toString()
				.equals(Constants.ACCOUNTTYPE_PERSON)) { // 如果当前登录为用户，则显示业务单申请来源信息
	...
	public String acceptpass() {
		...
		try {
			Business tempbusiness = businessService.getBusinessById(Long
					.valueOf(id));
			if (tempbusiness.getBusinessstate().toString().equals("1")) { // 防止重复提交
																			// (目前暂时不能完全防止重复提交)
				...
				if (pass_state != null && pass_state.equals("0")) { // 未通过
					...
					bus.setRemark(remark);
					bus.setBusinessstate(Long
							.valueOf(BusinessState.BUSINESS_ACCEPTNOT));
					...
					bus.setRemark(remark);
					bus.setBusinessstate(Long
							.valueOf(BusinessState.BUSINESS_ACCEPTSUCCESS));
					businessService.updateBusinessToState(bus);
				}
			}
	 ...
	 * @param request
	 */
	public void setBusinessCountrysandPorts(
			List<BusinessDesCountry> businessDesCountryList,
		...
		for (int j = 0; j < businessDesCountryList.size(); j++) {
			page_countrys += StringUtils.trimString(businessDesCountryList.get(
					j).getCountry())
					+ ",";
			page_ports += StringUtils.trimString(businessDesCountryList.get(j)
					.getPort()) + ",";
			page_selects += StringUtils.trimString(businessDesCountryList
					.get(j).getSelectindex()) + ",";
		}
	...
	public String selectupdataApply() throws AppException {
		...
		try {
			 ...
			 */
			System.out.println("============================="
					+ request.getParameter("inspection1"));
			business = businessService.getBusinessById(Long.parseLong(id));
			System.out.println("================================="
					+ business.getRegistry());
			...
			businessService
					.updateBusinessForSelect(bus, businessDesCountryList);
			business = businessService.getBusinessById(Long.valueOf(id));
	...
	public String businessRemind() throws AppException {
		...
		try {
			...
			String personid = (String) sessionmap
					.get(Constants.ACCOUNT_PERSON_ID);
			String role_type = (String) sessionmap
					.get(Constants.ACCOUNT_ALLOT_LEVEL);
			String own_orgid4 = (String) sessionmap
					.get(Constants.ACCOUNT_ORG_ID4);
			String isadmin = (String) sessionmap.get(Constants.ACCOUNT_ISADMIN); // 如果是超级管理员用户
			...
			if (accounttype != null && accounttype.equals("1")) { // 用户
				...
				map.put("state", operatortype);
				map.put("accountid", "-1"); // 用户只能受理自己机构所在的业务
				businessAcceptwaitCount = businessService
						.getBusinessListCOUNT(map);
				...
				if (isadmin != null && isadmin.equals("1")) {
					businessAllotCount = assignmentService
							.getBusinessByPersonIDCountSYS(map2);
				} else {
					businessAllotCount = assignmentService
							.getBusinessByPersonIDCount(personid, map2);
				}
				...
				businessCertificateCount = businessService
						.searchBusinessListCount(map3);
				...
				if (isadmin != null && isadmin.equals("1")) {
					businessSubmitCount = examineService
							.getAllBussinessCount(map4);
				} else {
				...
				map.put("state", businessstate);
				map.put("businessname", clientname);
				businessTempCount = businessService
						.getBusinessListClientCOUNT(map);
				...
				Integer checkCount = businessService
						.getBusinessListClientCOUNT(map);
				map.put("state", BusinessState.Business_ALLOTED);
				Integer checkCount1 = businessService
						.getBusinessListClientCOUNT(map);
			...
			request.setAttribute("businessAcceptwaitCount",
					businessAcceptwaitCount);
			request.setAttribute("businessAllotCount", businessAllotCount);
			request.setAttribute("businessCertificateCount",
